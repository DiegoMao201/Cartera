# ======================================================================================
# ARCHIVO: pages/📊_Análisis_Histórico.py (Versión con Análisis Automático)
# ======================================================================================
...
if df_historico.empty:
    st.warning("No se encontraron archivos de datos históricos con el formato 'Cartera_AAAA-MM.xlsx'."); st.stop()
...
# --- Filtrar el DataFrame principal por el rango de fechas para el análisis ---
...
# --- NUEVO: Sección de Análisis Automático ---
st.markdown("#### Análisis y Conclusiones del Período")
st.markdown('<hr style="border:1px solid #e0e0e0">', unsafe_allow_html=True)

# Análisis 1: Flujo de Efectivo (Ventas vs Cobros)
diferencia_flujo = total_cobrado - total_ventas
if diferencia_flujo >= 0:
    st.success(f"**✅ Gestión de Flujo Positiva:** En este período se ha cobrado **${diferencia_flujo:,.0f} más** de lo que se ha vendido. Esto indica una recuperación efectiva de cartera antigua y una buena salud de liquidez.")
else:
    st.warning(f"**⚠️ Crecimiento de Cartera:** Las ventas han superado a los cobros por **${abs(diferencia_flujo):,.0f}**. Es normal si la empresa está en crecimiento, pero vigila que la cartera por cobrar no se acumule excesivamente.")

# Análisis 2: Eficiencia de Cobro (DSO)
if dso_periodo <= 30:
    st.success(f"**✅ Eficiencia Óptima:** La rotación de cartera de **{dso_periodo:.0f} días** es excelente. Las facturas se están cobrando rápidamente, dentro del plazo de 30 días.")
elif dso_periodo <= 60:
    st.info(f"**👍 Eficiencia Aceptable:** La rotación de **{dso_periodo:.0f} días** es buena, aunque se podría trabajar en reducirla para optimizar el flujo de caja.")
else:
    st.error(f"**🚨 Alerta de Eficiencia:** La rotación de **{dso_periodo:.0f} días** es elevada. Se recomienda revisar y reforzar las políticas de cobro para reducir el tiempo que el dinero está en la calle.")
...
# --- NUEVO: Análisis de la tendencia del DSO ---
if len(df_final_graficos) > 1:
    dso_inicial = df_final_graficos['DSO'].iloc[0]
    dso_final = df_final_graficos['DSO'].iloc[-1]
    cambio_dso = dso_final - dso_inicial
    
    st.markdown("##### Diagnóstico de la Tendencia de Eficiencia")
    if cambio_dso < -1:
        st.success(f"**Tendencia Positiva:** La eficiencia de cobro ha **mejorado** notablemente. El tiempo promedio de cobro se ha **reducido en {abs(cambio_dso):.0f} días** durante este período.")
    elif cambio_dso > 1:
        st.warning(f"**Tendencia a Revisar:** La eficiencia de cobro ha **disminuido**. Ahora se tarda en promedio **{cambio_dso:.0f} días más** en cobrar que al inicio del período. Es importante analizar las causas.")
    else:
        st.info("**Tendencia Estable:** La eficiencia de cobro se ha mantenido estable durante el período seleccionado.")
