...
with col3: st.metric("🔄 Rotación (DSO)", f"{dso_periodo:.0f} días", help="Días promedio para cobrar facturas saldadas en este período.")
...
if diferencia_flujo >= 0: st.success(f"**✅ Flujo Positivo:** Se ha cobrado **${diferencia_flujo:,.0f} más** de lo que se ha vendido.")
else: st.warning(f"**⚠️ Crecimiento de Cartera:** Las ventas superaron a los cobros por **${abs(diferencia_flujo):,.0f}**.")
if dso_periodo <= 30: st.success(f"**✅ Eficiencia Óptima:** La rotación de **{dso_periodo:.0f} días** es excelente.")
...
df_final_graficos = pd.merge(ventas_mes, cobros_mes, on='mes', how='outer').fillna(0); df_final_graficos = pd.merge(df_final_graficos, dso_mes, on='mes', how='outer')
